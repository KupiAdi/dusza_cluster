...
def refresh_ui(app):
    if isinstance(app, InputApp):
        app.label.configure(
            text=translations[lang]["input_labels"]["enter_dir"])
        app.lang_label.configure(
            text=translations[lang]["messages"]["language"])
...
class InputApp(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(self.header_frame, text="Cluster manager", font=(
            "Arial", 18)).pack(side="left", padx=10, pady=10)
        self.lang_dropdown = ctk.CTkOptionMenu(self.header_frame, values=[
                                               "hu", "en"], command=lambda new_lang: update_language(new_lang, self))
        self.lang_label = ctk.CTkLabel(
            self.header_frame, text=translations[lang]["messages"]["language"])

        self.lang_dropdown.pack(side="right", padx=10, pady=10)
        self.lang_label.pack(side="right", pady=10)

        self.label = ctk.CTkLabel(
            self, text=translations[lang]["input_labels"]["enter_dir"])
        ...
        self.button = ctk.CTkButton(
            self, text="OK", command=self.open_main_app)
            ...
            if input:
                if os.path.isfile(f"{input}/.klaszter"):
                    ...
                    messagebox.showerror(
                        title="Error", message=translations[lang]["messages"]["cluster_not_found"])
...
class MainApp(ctk.CTk):
    ...
    def create_boxes(self, results):
        ...
        for i in range(total_boxes):
            ...
            if appearance_mode == "Dark":
                box = ctk.CTkFrame(
                    self.main_frame, fg_color="gray40", corner_radius=5)
            elif appearance_mode == "Light":
                box = ctk.CTkFrame(
                    self.main_frame, fg_color="white", corner_radius=5)
            ...
            title = ctk.CTkLabel(
                box, text=results[i]["pc"], font=("Arial", 22))
            title.pack(side="top", fill="x", padx=5)

            ctk.CTkLabel(box, text=f"Max CPU: {results[i]["max_cpu"]} Max RAM: {results[i]["max_ram"]}").pack(
                fill="x", padx=5)
            ctk.CTkLabel(box, text=f"{translations[lang]["messages"]["free"]} CPU: {results[i]["free_cpu"]} {translations[lang]["messages"]["free"]} RAM: {results[i]["free_ram"]}").pack(
                fill="x", padx=5)
            for j in range(len(results[i]["active_programs"])):
                ctk.CTkLabel(box, text=results[i]["active_programs"][j]).pack(
                    fill="x", padx=5)
    ...
    def create_header(self):
        ...
        ctk.CTkLabel(self.header_frame, text="Cluster manager", font=(
            "Arial", 22)).pack(side="left", padx=10, pady=10)
        self.lang_dropdown = ctk.CTkOptionMenu(self.header_frame, values=[
                                               "hu", "en"], command=lambda new_lang: update_language(new_lang, self))
        self.lang_dropdown.pack(side="right", padx=10, pady=10)
        self.lang_dropdown.set(lang)

        ctk.CTkLabel(self.header_frame, text=translations[lang]["messages"]["language"]).pack(
            side="right", pady=10)

        if lang == "hu":
            self.appearance_mode_dropdown = ctk.CTkOptionMenu(self.header_frame, values=[
                                                              translations[lang]["button_labels"]["dark"][0], translations[lang]["button_labels"]["light"][0]], command=self.change_appearance_mode_event)
            self.appearance_mode_dropdown.set(
                translations[lang]["button_labels"][appearance_mode.lower()][0])
            self.appearance_mode_dropdown.pack(side="right", padx=10, pady=10)
        elif lang == "en":
            self.appearance_mode_dropdown = ctk.CTkOptionMenu(self.header_frame, values=[
                                                              translations[lang]["button_labels"]["dark"], translations[lang]["button_labels"]["light"]], command=self.change_appearance_mode_event)
            self.appearance_mode_dropdown.set(appearance_mode)
            self.appearance_mode_dropdown.pack(side="right", padx=10, pady=10)

        ctk.CTkLabel(self.header_frame, text=translations[lang]["messages"]["theme"]).pack(
            side="right", pady=10)
    ...
    def create_buttons(self):
        ...
        for index, (button_text, command) in enumerate(buttons):
            button = ctk.CTkButton(
                self.footer_frame, text=button_text, command=command)
            button.grid(row=index//4, column=index %
                        4, padx=5, pady=5, sticky="ew")
    ...
    def show_input(self, function_name, input_keys):
        ...
        for input_name in input_names:
            input_entry = ctk.CTkEntry(
                self.input_frame, placeholder_text=input_name, justify='center')
            input_entry.pack(side="top", padx=10, pady=5,
        ...
        self.button_frame.pack(side="top", padx=10,
                               pady=5, expand=True, fill="both")

        self.ok_button = ctk.CTkButton(
            self.button_frame, text="OK", command=lambda: self.on_ok_button(function_name))
        self.ok_button.pack(side="left", padx=10, expand=True, fill="both")

        self.cancel_button = ctk.CTkButton(
            self.button_frame, text=button_labels["cancel"], command=self.cancel_input)
                ...
                case "add_computer":
                    if re.fullmatch(r'^[^\sÁÉÍÓÖŐÚÜŰáéíóöőúüű]+$', input_texts[0]):
                        if re.fullmatch(r'\d+', input_texts[1]) and re.fullmatch(r'\d+', input_texts[2]):
                            ...
                            messagebox.showerror(
                                title="Error", message=translations[lang]["messages"]["invalid_resource_value"])
                            correct = False
                    else:
                        messagebox.showerror(
                            title="Error", message=translations[lang]["messages"]["invalid_computer_name"])
                        correct = False
                case "stop_program":
                    self.stop_program(input_texts[0])
                case "modify_program":
                    self.modify_program(
                        input_texts[0], input_texts[1], input_texts[2], input_texts[3])
    ...
    def monitoring_btn(self):
        self.show_input(function_name="monitoring",
                        input_keys=["program_name"])

    def delete_computer_btn(self):
        self.show_input(function_name="delete_computer",
                        input_keys=["computer_name"])

    def add_computer_btn(self):
        self.show_input(function_name="add_computer", input_keys=[
                        "new_computer_name", "cpu", "ram"])

    def stop_program_btn(self):
        self.show_input(function_name="stop_program",
                        input_keys=["program_name"])

    def modify_program_btn(self):
        self.show_input(function_name="modify_program", input_keys=[
                        "program_name", "number_of_copies", "cpu", "ram"])

    def run_new_instance_btn(self):
        self.show_input(function_name="run_new_instance",
                        input_keys=["computer_name", "program_name"])

    def stop_instance_btn(self):
        self.show_input(function_name="stop_instance",
                        input_keys=["program_name"])
        ...
        for pc in os.listdir(dir):
            ...
            if os.path.isdir(pc_path) and os.path.isfile(os.path.join(pc_path, ".szamitogep_config")):
                with open(os.path.join(pc_path, ".szamitogep_config"), "r", encoding="utf-8") as conf_file:
                    ...
                    max_ram = int(config_lines[1].strip()) if len(
                        config_lines) > 1 else 0
        ...
        with open(f"{dir}/.klaszter", "r", encoding="utf-8") as f:
            ...
            for i in range(0, len(lines), 4):
                ...
                if expected_instances != actual_instances:
                    ...
                    ctk.CTkLabel(self.main_frame, text=f"{translations[lang]["messages"]["error_state"]} {prog} {translations[lang]["messages"]["expected_instances"]} {expected_instances}{translations[lang]["messages"]["actual_instances"]} {actual_instances}", font=(
                        "Arial", 16)).pack()
        if cluster_consistent:
            ctk.CTkLabel(self.main_frame, text=translations[lang]["messages"]["cluster_status_correct"], font=(
                "Arial", 16)).pack(pady=20)
        else:
            ctk.CTkLabel(self.main_frame, text=translations[lang]["messages"]["cluster_status_not_correct"], font=(
                "Arial", 16)).pack(pady=20)

        ctk.CTkLabel(self.main_frame, text=translations[lang]["messages"]["all_running_processes"], font=(
            "Arial", 16)).pack()
        ctk.CTkLabel(self.main_frame, text=f"{translations[lang]["messages"]["active"]}: {total_active} {translations[lang]["messages"]["inactive"]}: {total_inactive}", font=(
            "Arial", 16)).pack()

        ctk.CTkButton(self.main_frame, text="OK",
                      command=self.refresh_main_view).pack(pady=20)
    ...
    def monitoring(self, query):
        ...
        if query in program_instances:
            for widget in self.main_frame.winfo_children():
                widget.destroy()
            if lang == "hu":
                ctk.CTkLabel(self.main_frame, text=f"{query} {translations[lang]["messages"]["running_instances"]}:", font=(
                    "Arial", 16)).pack(pady=10)
            elif lang == "en":
                ctk.CTkLabel(self.main_frame, text=f"{translations[lang]["messages"]["running_instances"]} {query}:", font=(
                    "Arial", 16)).pack(pady=10)
            for instance in program_instances[query]:
                comp, unique_id, cpu_needed, ram_needed, status = instance
                ctk.CTkLabel(self.main_frame, text=f"{translations[lang]["messages"]["computer"]}: {comp}, {translations[lang]["messages"]["id"]}: {unique_id}, {translations[lang]["messages"]["resource_requirements"]}: CPU {cpu_needed}, RAM {ram_needed}, {translations[lang]["messages"]["status"]}: {status}", font=(
                    "Arial", 16)).pack()
            ctk.CTkLabel(self.main_frame, text=f"{translations[lang]["messages"]["total_running_instances"]}: {len(program_instances[query])}", font=(
                "Arial", 16)).pack(pady=10)
            ctk.CTkButton(self.main_frame, text="OK",
                          command=self.refresh_main_view).pack(pady=10)
        else:
            messagebox.showerror(
                title="Error", message=translations[lang]["messages"]["no_running_instance_with_this_name"])
    ...
    def delete_computer(self, pc):
        ...
        if pc in os.listdir(dir) and os.path.isfile(f'{dir}/{pc}/.szamitogep_config'):
            if os.listdir(f"{dir}/{pc}") != ['.szamitogep_config']:
                for widget in self.main_frame.winfo_children():
                    widget.destroy()
                ctk.CTkLabel(self.main_frame, text=translations[lang]["messages"]["computer_not_empty"], font=(
                    "Arial", 16)).pack(pady=10)
                ctk.CTkLabel(self.main_frame, text=translations[lang]["messages"]["running_programs"], font=(
                    "Arial", 16)).pack()
                for k in os.listdir(f"{dir}/{pc}"):
                    if ".szamitogep_config" not in k:
                        ctk.CTkLabel(self.main_frame, text=k,
                                     font=("Arial", 16)).pack()
                        programok = open(f"{dir}/{pc}/{k}", encoding="utf-8")
                        for line in programok:
                            if cycle < 2:
                                cycle += 1
                                ctk.CTkLabel(self.main_frame, text=line.replace(
                                    "\n", ""), font=("Arial", 16)).pack()
                    ctk.CTkLabel(
                        self.main_frame, text="-------------------------", font=("Arial", 16)).pack()
                    cycle = 0
                ctk.CTkButton(self.main_frame, text="OK",
                              command=self.refresh_main_view).pack(pady=10)
            else:
            ...
            messagebox.showerror(
                title="Error", message=translations[lang]["messages"]["no_computer_found"])

    def add_computer(self, new_pc, performance1, performance2):
        if new_pc in os.listdir(dir):
            messagebox.showerror(
                title="Error", message=translations[lang]["messages"]["computer_already_exists"])
        else:
    ...
    def stop_program(self, program):
        ...
        if not found:
            messagebox.showerror(
                title="Error", message=translations[lang]["messages"]["no_program_found"])
                    ...
                    if program == j:
                        ...
                        for k in rows:
                            ...
                            if k.strip() == program.split('-')[0]:
                                self.modify_line(
                                    f"{dir}/.klaszter", n+1, str(int(rows[n+1])-1))
                        exists = True
                        break
        if not exists:
            messagebox.showerror(
    ...
    def run_new_instance(self, new_pc, new_program):
        ...
        def update_instances_count(program_name, instances_index):
            ...
            lines[instances_index] = str(
                int(lines[instances_index].strip()) + 1) + "\n"
        ...
        def check_resources_and_create_file(program_name, new_pc):
            ...
            instances, cpu_needed, ram_needed, instances_index = get_program_resources(
                program_name)
        ...
        if not check_resources_and_create_file(new_program, new_pc):
            messagebox.showerror(
                title="Error", message=translations[lang]["messages"]["invalid_computer_program_name"])
